...
namespace CST
{
    public class TPL22 : VpbScene
    {
        ...
        [SerializeField]
        private TMPro.TextMeshProUGUI _txtMesh_pro_Right;
        ...
        {
            //D
            StartCoroutine(SetAnimTitle());
        }


        IEnumerator SetAnimTitle()
        {
            ...
            if (PopupManager.check_showAnim == 0)
            {
                ...
                if (System.IO.File.Exists(pathD))
                {
                    ...
                    if (gameData.title != "")
                    {
                        ...
                        _gameController.GetComponent<Game_controller>()._bgtitle.transform.GetChild(0).GetComponent<Text>().text = gameData.title;
                    }
                    }
                    else
                    {
        ...
        protected void SampleQuest()
        {
            ...
            if (System.IO.File.Exists(path))
            {
                ...
                gameData = JsonUtility.FromJson<MatData>(fileContents);
                ...
                _txtMesh_pro_Right.text = gameData.question;
            }
        ...
        void setupText()
        {
            ...
            while (isOK)
            {
                ...
                if (isOK)
                {
                    ...
                    str = str.Replace("<#>" + arr[k] + "</#>", " <link>vvvvvvvv</link> ");
                }
                k++;
            }
        ...
        IEnumerator SetTMP()
        {
            ...
            for (int i = 0; i < textInfo.linkCount; i++)
            {
                ...
                x_ = vxy.x * 0.02f - 6.95f;
                y_ = vxy.y * 0.02f - 8.4f;
                target_.transform.position = new Vector3(x_, y_, 0);
                target_.transform.localScale = new Vector3(0.85f, 0.83f, 0);
        ...
        public void getAnswer()
        {
            ...
            int i = 0;
            answersU.Clear();
            foreach (string s in gameData.right_answer)
            {
                answersU.Add("");
                if (targetArr[i].GetComponent<VpbField>().eObject != null)
                {
                    answersU[j] = "" + targetArr[i].GetComponent<VpbField>().eObject.GetComponent<VpbDragObject>().eData;
                }
                j++;
                i++;
            }
        ...
        public void CalScore()
        {
            ...
            for (int i = 0; i < answersU.Count; i++)
            {
                if (answersU[i] != right_answers[i])
                {
                    isTrue = false;
                    break;
                }
            ...
            {
                score = 0;
                PopupManager.listAnswer[PopupManager.index].star_count = 0;
            }
            else
            {
                //--demo 
                score = 1;
                PopupManager.listAnswer[PopupManager.index].star_count = 1;
                // int index = PlayerController.index - 1;
                // PopupManager.listAnswer[index] = true;
                // Debug.Log(index + ":" + PopupManager.listAnswer[index]);
            }
